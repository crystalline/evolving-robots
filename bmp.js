...
try {
    ...
    console = {log: cprint, error: cprint, info: cprint};
}

//Buffer shim for pixelscript and the browser
...
function encodeBMP(w, h, imgDataData) {
    if (typeof w === 'number' && typeof h === 'number' && imgDataData.length === (w*h*4)) {
        var encoder = new BmpEncoder(w, h, imgDataData);
        return encoder.encode();
    } else {
        console.log('bmp.encode(width, height, imageData) was called with wrong arguments. Check that imgDataData.length === width*height*4');
    }
}
